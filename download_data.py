...
def DownloadUrl(url, corpus, max_attempts=5, timeout=5):
  ...
  while attempts < max_attempts:
    try:
      pos = url.index('_')
      true_url = url[pos + 2:len(url)];
      req = requests.get(true_url, allow_redirects=False, timeout=timeout)
...
def ImageMode(corpus, request_parallelism):
  """Downloads the URLs for the specified corpus.
  ...
  missing_urls = []
  for dataset in datasets:
    print 'Downloading Images for the %s set:' % dataset
    ...
    results = p.imap_unordered(ImageMapper, izip(urls, repeat(corpus)))
    # print(results)
    progress_bar = ProgressBar(len(urls))
...
def ImageMapper(t):
  """Downloads an Image and checks that metadata is available for the Image.

  Args:
    t: a tuple (url, corpus).

  Returns:
    A pair of imageURL and caption.
  ...
  ok = DownloadImage(url, corpus)
  return url, ok


def DownloadImage(url, corpus, max_attempts=2, timeout=5):
  try:
    text = ''
    downloaded_images = []
    ok = 0
    url_hash = Hashhex(url)
    with open('%s/downloads/%s.html' % (corpus, url_hash)) as f:
      text = f.read()
    # print(text)
    
    image_path = '%s/images/%s' % (corpus, url_hash)
    caption_file = '%s/captions/%s' % (corpus, url_hash)

    if not os.path.exists(image_path):
      os.mkdir(image_path)
      
    images = ParseHtmlForImage(text, corpus, url)
    ...
    for image in images:
      ...
      downloaded_indice = []
      downloaded_images.append({'src':downloaded_indice, 'caption':image_caption})
      for image_url in image_urls:
        try:
          i = i + 1
          
          with open('%s/%s-%d' % (image_path, url_hash, i)) as f:
            
            downloaded_indice.append(i)
            # print(i)
            continue
        except IOError:
          attempts = 0
          # print(i)
          # print("%s \n %s"%(image_url,image_caption))
          while attempts < max_attempts:
            try:
              # req = requests.get(image_url, allow_redirects=False, timeout=timeout)
              # print('%d %s' % (i, image_url))
              headers = {"User-Agent":"Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/45.0.2454.101 Safari/537.36"}
              response = requests.get(image_url, headers=headers, stream=True, allow_redirects=False, timeout=timeout)
              # print('%d  %s   %s/images/%s-%d' % (attempts, image_url, corpus, url_hash, i))
              if response.status_code == requests.codes.ok:
                with open('%s/%s-%d' % (image_path, url_hash, i), 'wb') as f:
                  for chunk in response.iter_content(128):
                    f.write(chunk)
                  ok = ok + 1
                  downloaded_indice.append(i)
                  break
              elif (response.status_code in [301, 302, 404, 503] and attempts == max_attempts - 1):
                break
            except requests.exceptions.ConnectionError:
              # time.sleep(300)
              print("1 %s %s %d" % (image_url, image_caption, attempts))
            except requests.exceptions.ContentDecodingError:
  ...
  if len(downloaded_images) > 0:
    with open(caption_file, 'w') as f:
      for image in downloaded_images:
        downloaded_indice = image['src']
        downloaded_caption = image['caption']
        # print(downloaded_caption)
        for index in downloaded_indice:
          f.write('%d' % index)
          f.write(' ')
        f.write('\n')
        f.write(downloaded_caption)
        f.write('\n')
  return ok


def ParseHtmlForImage(text, corpus, url):
  corpus = corpus[corpus.rfind('/') + 1:len(corpus)]
  parser = html.HTMLParser(encoding=chardet.detect(text)['encoding'])
  tree = html.document_fromstring(text, parser=parser)
  # print(text)
  
  image_imgdiv_selectors = {
      'cnn': [
          '//div[contains(@class, "cnnStoryPhotoBox")]//div[contains(@class, "cnnImgChngr")]//img//@src'
      ],
      'dailymail': [
          '//div[contains(@class, "thinFloatRHS") or contains(@class, "thinCenter") or contains(@class, "artSplitter") ]'
      ]
  }

  if corpus == 'dailymail':
    images = [];
    xpaths = map(tree.xpath, image_imgdiv_selectors[corpus])
    elements = list(chain.from_iterable(xpaths))
    # print('33 %d'%(len(elements)))
    for e in elements:
      image_captions = [];
      p_eles = e.getiterator('p')
      for p_ele in p_eles:
        if p_ele.text is not None and p_ele.get('class') == 'imageCaption':
          # print(p_ele.text)
          image_captions.append(p_ele.text.encode('utf-8'))
          break
      
      if len(image_captions) > 0:
        image_urls_ele = e.getiterator('img')
        image_urls = [ele.get('src').encode('utf-8') for ele in image_urls_ele]
        # print(len(image_captions))
        # print(len(image_urls))
        # print(image_captions[0])
        image_caption_text = image_captions[0]
        # print(image_caption_text)
        images.append({'src':image_urls, 'caption':image_caption_text})
    return (images)
  elif corpus.rfind('cnn') > -1:
    return None
  
  
def DownloadImage2(url_hash, corpus, max_attempts=5, timeout=5):
  """Downloads a URL.
  ...
  try:
    ...
    with open('%s/downloads/%s.html' % (corpus, url_hash)) as f:
      text = f.read()
    # print(text)
    image_urls_text, image_captions_text = ParseHtmlForImage(text, corpus)
    
    image_urls = image_urls_text.split('\n\n')
    image_captions = image_captions_text.split('\n\n')
    downloaded_indice = [];
    ...
    for i in range(len(image_urls)):
      ...
      try:
        with open('%s/images/%s-%d' % (corpus, url_hash, i)) as f:
          downloaded_indice.append(i)
          continue
      except IOError:
        attempts = 0
        ...
        while attempts < max_attempts:
          try:
            ...
            # print('%d  %s   %s/images/%s-%d' % (attempts, image_url, corpus, url_hash, i))
            if response.status_code == requests.codes.ok:
              with open('%s/images/%s-%d' % (corpus, url_hash, i), 'wb') as f:
                for chunk in response.iter_content(128):
                  f.write(chunk)
                ok = ok + 1
                downloaded_indice.append(i)
    ...
    pass
  with open('%s/captions/%s' % (corpus, url_hash), 'w') as f:
    for index in downloaded_indice:
      f.write('%d' % index)
      f.write('\n')
      f.write(image_captions[index])
      f.write('\n')
  return ok


def ParseHtmlForImage2(text, corpus):
  """Parses the HTML of a news story.
  ...
  image_src_selectors = {
      ...
      'dailymail': [
          '//div[contains(@class, "thinFloatRHS") or contains(@class, "thinCenter") or contains(@class, "artSplitter")]//img//@src'
      ]
  }

  image_caption_selectors = {
      'cnn': [
          '//div[contains(@class, "cnnStoryPhotoBox")]//div[contains(@class, "cnnImgChngr")]//img//@alt'
      ],
      'dailymail': [
          '//div[contains(@class, "thinFloatRHS") or contains(@class, "thinCenter") or contains(@class, "artSplitter")]//p[contains(@class, "imageCaption")]/text()'
      ]
  }

  def ExtractImage(selector):
    """Extracts a list of paragraphs given a XPath selector.
    ...
    xpaths = map(tree.xpath, selector)
    elements = list(chain.from_iterable(xpaths))
    lines = [e.encode('utf-8') for e in elements]

    lines = map(str.strip, lines)
    lines = [s for s in lines if s and not str.isspace(s)]

    return lines

  image_urls = ExtractImage(image_src_selectors[corpus])
  image_captions = ExtractImage(image_caption_selectors[corpus])
  # print(image_urls)
  # print(image_captions)
  image_urls_text = '\n\n'.join(image_urls)
  image_captions_text = '\n\n'.join(image_captions)

  return (image_urls_text, image_captions_text)
...
def main():
  ...
  parser.add_argument(
      '--mode', choices=['store', 'generate', 'download', 'urls', 'remove', 'image'],
      default='generate')
  parser.add_argument('--request_parallelism', type=int, default=30)
  parser.add_argument('--context_token_limit', type=int, default=2000)
  args = parser.parse_args()

  base_dir = '..'
  args.corpus = '%s/%s' % (base_dir, args.corpus)
  ...
  if args.mode == 'store':
    ...
    ImageMode(args.corpus, args.request_parallelism)
